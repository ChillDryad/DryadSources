...
export default class Controller {
  ...
  private client = new NetworkClientBuilder().addRequestInterceptor(interceptor).build()
  ...
  async getSearchResults(query: DirectoryRequest): Promise<PagedResult> {
    ...
    if(query.filters?.genres) 
      query.filters.genres.map((genre: string, i: number) => { params[`genre[${i}]`]=genre })
    ...
    const response: NetworkResponse = await this.client.get(`${this.BASE}/search${search}/page/${page}`, {params})
